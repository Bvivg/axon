...
- Заголовок карточки/страницы есть — описание под ним не пишем (никаких `CardDescription`/подзаголовков с поясняющим текстом). Название говорит само за себя; пояснения, если они правда нужны, — это подсказка прямо у нужного поля (например, warning под инпутом email при его смене), а не отдельный блок текста под заголовком.
