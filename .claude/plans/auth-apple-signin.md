...
Имя едет в **отдельном поле контракта**: `CompleteOAuthRequest.display_name`
(`optional`, номер 4). Route handler кладёт его в query редиректа рядом с
`code`/`state`, страница callback передаёт в `CompleteOAuth`, а флоу подставляет
в профиль — но только если провайдер своего имени не дал. Приоритет именно
такой: то, что сказал провайдер, браузером не переписывается.

Провайдер Apple имени не видит вообще и не может: Apple отдаёт его тому, кто
принял callback, то есть клиенту. `appleProvider.Exchange` возвращает профиль без
`DisplayName`, и это зафиксировано тестом.

> Изначально (в рамках задачи по Apple) контракт менять было нельзя — `shared/proto`
> правился параллельной сессией, — и имя ехало упакованным в `code`:
> `base64url(JSON{code,name})`. Упаковка снята; протокол поверх чужого поля
> прожил ровно до того момента, когда контракт стало можно тронуть.

Имя — самоописание пользователя и у Apple тоже не проверяется, так что подделать
через это поле можно ровно то, что и так вводится руками; адрес всегда берётся из
подписанного `id_token`, а `domain.ValidateDisplayName` отбрасывает негодное
значение, не роняя вход (существующее поведение `resolveOauthUser`).
