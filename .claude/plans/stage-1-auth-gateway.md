...
Открытый вопрос: имя пользователя Apple присылает ровно один раз, при первой
авторизации, а `CompleteOAuth` несёт только provider/code/state. Сейчас имя
упаковано вместе с кодом — `base64url(JSON{code,name})`. Работает, но это
протокол поверх чужого поля; правильное место — отдельное `optional`-поле в
контракте.
