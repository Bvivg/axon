...
### OAuth — ✅ ВЫПОЛНЕНО (fake + Google + GitHub; Apple отдельным шагом)
...
Apple остаётся невыполненным намеренно: `client_secret` у него — ES256-JWT с
ротацией, профиль приходит только в `id_token` и только при первой авторизации,
а callback — `form_post`. В реестре его нет, поэтому он ведёт себя как любой
ненастроенный провайдер.
