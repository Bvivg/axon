...
Фиксируются по ходу работы, как требует `rules/plans.md`.

- **`seq` — счётчик в строке комнаты, а не `bigserial`.** Последовательность
  выдаёт номера в одном порядке, а коммитятся транзакции в другом: клиент,
  спросивший `since=N` в момент, когда меньший `seq` ещё не закоммичен, потерял
  бы сообщение навсегда и молча. Инкремент `rooms.next_seq` берёт row lock, и
  порядок выдачи совпадает с порядком видимости. Цена — сериализация записи в
  пределах одной комнаты, что для чата и есть желаемое поведение.
- **Уникальность по `client_id` — частичный индекс** `WHERE client_id <> ''`:
  не каждое сообщение приходит с идентификатором клиента, и обычный уникальный
  индекс склеил бы все такие в одно. Вставка — `ON CONFLICT DO NOTHING` плюс
  повторный `SELECT`: поднятая ошибка ограничения оборвала бы транзакцию.
- **Шаги 5 и 6 — один коммит.** Локальный hub и шина — одна абстракция
  (`pubsub.Bus` с реализациями `Memory` и `Redis`); коммит с hub'ом без шины был
  бы коммитом с заведомо неверной доставкой между инстансами.
- **Publisher проверяется unit-тестом с фейковым sink**, а не брокером в
  integration-наборе chat. Контракт `shared/pkg/kafka` уже проверяется против
  настоящего брокера в `shared/integration`; второй брокер на набор — минуты в
  CI за повторную проверку чужого кода.
- **Integration-тесты репозитория приехали шагом 2, а не шагом 9.** Смысл этого
  репозитория — гарантии порядка под конкурентной записью; коммитить его
  непроверенным значило бы коммитить надежду.
- **Шаг 10 вырос за пределы «два контекста в одной комнате».** Веб-клиент
  получил редиректы-гварды (`lib/auth/guards.ts`), и вместе с ними изменилась
  посадочная страница после входа: лобби чата, а не профиль. Поэтому в
  `auth.spec.ts` переписаны ожидания, а сценарии на сами редиректы добавлены
  туда же — они про сессию, а не про чат.
